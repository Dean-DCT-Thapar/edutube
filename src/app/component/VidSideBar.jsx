...
export default function Sidebar(props) {
  ...
  return (
    <div className="w-96 h-screen p-4 bg-white absolute right-0 shadow-lg border-t-4 border-black overflow-y-auto">
      <h1 className="text-xl font-bold mb-4 text-black border-b pb-2">Course Content</h1>
      <ul> 
        {data.map((chapter) => (
          <li key={chapter.chapter_number} className="mb-2">
            <hr className="border-t border-black" />
            <div
              className="flex items-center justify-between cursor-pointer p-4 bg-white hover:bg-[#eeeeee] hover:text-white border-b border-black"
              onClick={() => toggleExpand(chapter.chapter_number)}
            >
              <span className="text-black font-semibold text-xl border-t border-black">{chapter.chapter_name}</span>
              ...
              <ul className="ml-0 mt-2">
                {chapter.lectures.map((lecture) => (
                  <Link key={`${chapter.chapter_number}-${lecture.lecture_number}`} href={`/course_page/${props.course_id}?chapter=${chapter.chapter_number}&lecture=${lecture.lecture_number}`}>
                    <li key={lecture.lecture_number} className={`p-4 items-center hover:bg-[#eeeeee] hover:text-white border-b border-black ${chapter.chapter_number === parseInt(chapterNumber) && lecture.lecture_number === parseInt(lectureNumber) ? 'bg-[#eeeeee]' : 'bg-white '}`}>
