...
export async function GET(request) {
    try {
        ...
        const response = await fetch(`${getBackendUrl()}/api/admin/course-templates${queryString ? `?${queryString}` : ''}`, {
            method: 'GET',
            headers: {
...
export async function POST(request) {
    try {
        ...
        const response = await fetch(`${getBackendUrl()}/api/admin/course-templates`, {
            method: 'POST',
            headers: {
                'Authorization': `Bearer ${adminToken.value}`,
                'Content-Type': 'application/json',
